...
// SELECT * FROM tbltransaction WHERE user_id = userId
export const getTransactions = async(req, res) => {
    try {
        ...
        const transactions = await pool.query({
            text: `SELECT * FROM tbltransaction WHERE user_id = $1 AND createdat BETWEEN $2 AND $3 AND (description ILIKE '%' || $4 || '%' OR 
            status ILIKE '%' || $4 || '%'  OR source ILIKE '%' || $4 || '%') ORDER BY id DESC`,
...
export const getDashboardInformation = async(req, res) => {
    try {
        ...
        const result = await pool.query({
            text: `SELECT EXTRACT(MONTH FROM createdat) AS month, type, SUM(amount) AS 
            totalAmount FROM tbltransaction WHERE user_id = $1 AND createdat BETWEEN $2 
            AND $3 GROUP BY EXTRACT(MONTH FROM createdat), type`,
...
// Make a transaction or withdrawal outside of our accounts
export const addTransaction = async(req, res) => {
    try {
        ...
        if(Number(amount) <= 0)
            return res.status(403).json({status:"failed", message:"Amount should be grater than 0."});
...
export const transferMoneyToAccount = async(req, res) => {
    try {
        ...
        // Check account details for the "to account"
        const toAccountResult = await pool.query({
            text: `SELECT * FROM tblaccount WHERE id = $1`,
            values: [to_account],
        });

        const toAccount1 = toAccountResult.rows[0];

        if (!toAccount1) {
            return res.status(404).json({status:"failed", message:"Destination account not found."});
        }
        ...
        res.status(201).json({status: "success", message: "Transfer completed successfully."});
