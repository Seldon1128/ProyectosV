...
export const createAccount = async(req, res) => {
    try {
        ...
        res.status(201).json({status: "success", message: account.account_name + " Account created succesfully", data: account,});
...
export const addMoneyToAccount = async(req, res) => {
    try {
        ...
        res.status(200).json({status: "success", message: "Operation completed succesfully", data: accountInformation,});
    } catch (error) {
        console.log(error);
        res.status(500).json({status:"failed", message: error.message});
    }
